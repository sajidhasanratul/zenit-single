    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.serviceMetrics) {
          setMetrics(data.serviceMetrics);
        }
      }
      const metricRes = await fetch('/api/analytics/service-metric');
      if (metricRes.ok) {
                ...
                <div key={item.id} className="flex justify-between items-start border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                  <div className="space-y-2.5">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5 bg-cyan-50 border border-cyan-200 text-cyan-800 px-2 py-0.5 rounded text-[9px] font-mono font-bold">
                        <span>👁️ {metrics[item.id]?.views || 0} views</span>
                        <span>•</span>
                        <span>🖱️ {metrics[item.id]?.clicks || 0} clicks</span>
                        <span>•</span>
                        <span>CTR: {metrics[item.id]?.views ? ((metrics[item.id].clicks / metrics[item.id].views) * 100).toFixed(1) : '0.0'}%</span>
                      </div>
                    </div>
